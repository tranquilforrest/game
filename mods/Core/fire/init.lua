...
minetest.register_node("fire:flame", {
	...
	on_rightclick = crafting.make_on_rightclick('fire', 1, { x = 8, y = 1 }),
...
minetest.register_node("fire:flame_permanent", {
...
minetest.override_item("default:coalblock", {
	after_destruct = function(pos, oldnode)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "fire:flame_permanent" then
			minetest.remove_node(pos)
		end
	end,
	on_ignite = function(pos, igniter)
		...
		if minetest.get_node(flame_pos).name == "air" then
			minetest.set_node(flame_pos, {name = "fire:flame_permanent"})
		end
	end,
})]]
...
if flame_sound then
	...
	function fire.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"fire:flame", "fire:flame_permanent"}
		)
		-- Total number of flames in radius
		local flames = (num["fire:flame"] or 0) +
			(num["fire:flame_permanent"] or 0)
		-- Stop previous sound
...
minetest.register_abm({
	...
	nodenames = {"fire:flame", "fire:flame_permanent"},
...
if not fire_enabled then
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if p then
				minetest.set_node(p, {name = "fire:flame"})
			end
		end,
	})
