...
local button_prefix = "sfinv_options_"
...
sfinv_options.register_button = function(name, def)
	buttons[name] = def
...
sfinv.register_page("sfinv_options:buttons", {
	title = S("Options"),
	...
	get = function(self, player, context)
		...
		if buttons_num > MAX_BUTTONS then
			f = f .. "textlist[0,0;7.8,8;sfinv_options_textlist;"
		end
		...
		if buttons_num > MAX_BUTTONS then
			...
			f = f .. "]"
			f = f .. "button[0,8;8,1;sfinv_options_action;"..minetest.formspec_escape(S("Go")).."]"
		end
	...
	on_player_receive_fields = function(self, player, context, fields)
		...
		if fields.sfinv_options_action then
			local button = buttons[player_selections[player_name]]
			if button ~= nil and button.action ~= nil then
				button.action(player)
			end
		elseif fields.sfinv_options_textlist then
			local explode = minetest.explode_textlist_event(fields.sfinv_options_textlist)
